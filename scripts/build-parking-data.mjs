...
const shortAddr = (r) => {
  ...
  return (
    full
      .split(" ")
      // 숫자로 시작하는 마디는 버리되, 한글로 끝나면 남긴다 — "1428"·"118-1"·"301(위탁)"은
      // 번지고, "5길"·"10길"은 띄어쓰기로 쪼개진 도로명이라 빼면 주소가 뭉툭해진다.
      .filter((t) => t && (!/^\d/.test(t) || /[가-힣]$/.test(t)))
      // 한글 뒤에 번지가 띄어쓰기 없이 붙은 것만 떼낸다 ("유수암리648" → "유수암리").
      // 앞이 한글일 때만 떼야 "한림남1길"·"하귀1리"처럼 숫자가 이름의 일부인 마디가 안 망가진다.
      .map((t) => t.replace(/(?<=[가-힣])\d+(-\d+)?$/, ""))
      .join(" ") || null
  );
};
