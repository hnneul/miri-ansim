// 판독 결과 적기 —
//   node --experimental-strip-types scripts/left-turn-set.mjs 덕수1교차로 비보호 2026-03-09
//
...
const [찾을것, verdict, shotAt] = process.argv.slice(2);
if (!찾을것 || !값.includes(verdict)) {
  console.error(`사용법: <이름 또는 좌표키> <${값.join("|")}> [촬영일 YYYY-MM-DD]`);
  process.exit(1);
}
...
console.log(`${d[k].label} → ${verdict} (촬영 ${d[k].shotAt ?? "미기록"})`);
console.log(`  판정 ${v.filter((x) => x.verdict).length} / ${v.length}곳`);
