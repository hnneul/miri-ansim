...
export function tradeoff(
  ...
  if (pick === route.id)
    return 분 > 0
      ? `일반 길보다 ${분}분 빠르고, 부담도 적어요`
      : 분 < 0
        ? `일반 길보다 ${-분}분 더, 대신 훨씬 편해요`
        : "시간은 같은데 부담이 더 적어요";
...
const 저쪽성격: Record<RiskType, string> = {
  ...
  narrowRoad: "차 한 대 폭으로 좁아지는 데가 있는 길인데",
...
const 각오: Record<RiskType, string> = {
  ...
  narrowRoad: "마주 오는 차 있으면 한쪽이 비켜야 해요. 넓은 데서 기다렸다 가시면 돼요.",
 ...
 *
 * **비교표(RouteStats 의 좌회전·회전교차로)를 쓰지 않는다.** 그 값들은 부담점수에 한 점도
 * 들어가지 않기 때문이다 — risksOf 가 만드는 요인은 급커브·좁은 교행·고속주행 셋뿐이다
 * (lib/route.ts). 화면은 부담점수를 큰 글씨로 띄워놓는데 음성만 점수와 무관한 축으로 이유를
