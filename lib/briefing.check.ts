...
assert.equal(tradeoff("safe", 느림, 빠름, "짧은 길"), "짧은 길보다 9분 더, 대신 훨씬 편해요");
...
/*
 * **상대를 부르는 이름은 화면이 정한 걸 그대로 쓴다.** 여기서 "빠른 길"로 못 박고 있었는데,
 * 화면은 그 자리를 "짧은 길"이라 부른다 (app/route/page.tsx 기본이름). 카드와 이 줄이 같은
...
assert.equal(tradeoff("fast", 느림, 빠름, "짧은 길"), "긴장할 구간이 더 많은 길이에요");
for (const 말 of [tradeoff("safe", 빠름, 느림, "안심 길"), tradeoff("fast", 느림, 빠름, "짧은 길")])
...
assert.equal(tradeoff("single", 느림, 빠름, "짧은 길"), "");
...
assert.equal(tradeoff("safe", { id: "safe", durationMin: null }, 빠름, "짧은 길"), "");
