 ...
 *
 * 20 으로 줄여봤다가 40 으로 되돌렸다. 축척은 **마지막 한 곳까지 담도록** 잡히므로(fitM)
 * 값을 줄이면 지도가 그만큼 당겨져 핀 사이가 벌어지긴 한다. 대신 목록도 같이 반토막이 난다 —
 * 제주시청 1km 안에 823곳인데 스무 곳만 보여주는 건 "이 동네에 이만큼 깔려 있다"를 깎는다.
 *
