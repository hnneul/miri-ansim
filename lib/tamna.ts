 ...
 *
 * 40 에서 20 으로 줄였다. 제주시청 1km 안에 823곳이라 40개는 축척을 아무리 맞춰도 서로를
 * 가렸다 — 축척은 **마지막 한 곳까지 담도록** 잡히는데(fitM), 그 40번째가 멀수록 지도가
 * 물러나면서 안쪽 서른몇 개가 한 덩어리로 뭉친다. 20 이면 그 바깥 테두리가 안으로 당겨져
 * 축척도 같이 당겨지고, 핀 사이가 벌어진다.
 *
 * 지도만 20 으로 줄이고 목록은 40 을 두는 방법도 있었지만, 그러면 축척이 여전히 40 기준이라
 * 정작 뭉치는 건 그대로다. 자르려면 여기서 잘라야 한다.
 */
const SHOP_CAP = 20;
