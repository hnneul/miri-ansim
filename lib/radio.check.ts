...
// --- ③ ②칸: 왜 이 길인지 — 세 갈래 ---
//
// (가) 추천한 길을 골랐다. **일반 길에는 있고 이 길에는 없는 부담**을 말한다.
//
// 비교표의 좌회전·회전교차로는 쓰지 않는다 — 추천점수에 한 점도 안 들어가는 축이라
// (lib/route.ts risksOf), 화면이 큰 글씨로 띄운 점수와 음성이 서로 다른 얘기를 하게 된다.
const 추천이유 = 대본()[1];
// 상대를 부르는 이름은 화면 카드가 붙인 그것이다 (app/route/page.tsx 기본이름)
assert.ok(추천이유.includes("일반 길은"), `화면은 "일반 길"이라 부르는데 대본이 딴 이름을 썼다: ${추천이유}`);
...
/*
 * ★ **다른 경로를 도로 이름이나 지시어로 부르지 않는다.**
 *
 * 도로 이름은 제주가 처음인 사람에게 뜻이 없고, "저쪽"은 소리만으로 무엇을 가리키는지 모른다.
 * 부르는 이름은 화면 카드와 같은 "일반 길"이다 (app/route/page.tsx 기본이름).
 */
for (const 종류 of [대본(), 대본(fast, safe)])
  for (const 칸 of 종류.slice(1)) {
    assert.ok(!칸.includes("저쪽"), `가리키는 말을 썼다: ${칸}`);
    assert.ok(!칸.includes("다른 길"), `화면에 없는 이름으로 상대를 불렀다: ${칸}`);
