...
export type ScoreResult = {
   ...
   *
   * 예전에는 unclear 가 하나 더 있었다 — 차이는 있는데 시간과 맞바꿔야 해서 단정을 미루던 자리다.
   * 추천을 추천점수 하나로 정하면서 사라졌다: 5% 를 넘는 차이는 그냥 높은 쪽이 추천이다.
   *
...
/**
 * "권할 만한 길"의 선 — **추천점수** 기준이다 (이상이면 권할 만함).
 *
 * **추천을 정하는 값이 아니다.** 추천은 두 길의 추천점수를 견줘 높은 쪽으로 간다(scoreRoutes).
 * 이 값은 한 길만 놓고 "이 정도면 괜찮은 길인가"를 말할 때 쓴다 — 지금은 AI 대본 프롬프트가
 * 그 뜻으로 읽어 간다 (lib/ai.ts).
 */
 ...
 * 실측에서 평화로 경유 경로들이 그 한 요인 때문에 43~66점까지 떨어졌다.
 *
 ...
 *
 * 아래 scoreRoutes 의 비교식은 계속 부담으로 본다. 안쪽까지 뒤집으면 비율로 쓴 규칙이 뜻을
 * 잃기 때문이다 — "큰 값의 5% 이내면 같다"는 판정이 축을 뒤집으면 정확히 반대가 된다
 * (부담 80 vs 84 는 같다고 보는데, 추천점수 20 vs 16 은 다르다고 본다 — 같은 4점 차이다).
 * 어느 쪽이 높은지를 가리는 데는 축이 상관없다: 부담이 낮은 쪽이 곧 추천점수가 높은 쪽이다.
 *
...
export function scoreRoutes(
  ...
  /*
    **추천점수가 높은 쪽을 추천한다.** 그 이상도 이하도 아니다.

    예전에는 시간까지 봤다 — 빠른 길의 부담이 임계값(50) 이하로 이미 편하면, 안심 길이 더
    편해도 시간을 아끼는 쪽을 골랐다. 그러면 **화면이 자기 모순에 빠진다**: 추천점수 89 옆에
    80짜리가 추천 배지를 달고 앉는다. "추천점수"라고 이름 붙여 놓고 그 점수로 추천을 안 한 셈이라,
    틀린 건 추천이 아니라 그 이름이었다.

    시간을 버린 게 아니라 **자리를 옮겼다.** 두 카드의 소요시간, 근거 화면의 맞바꿈 한 줄,
    비교표가 그 정보를 그대로 들고 있다. 이 앱은 길을 대신 골라주는 앱이 아니라 그 길에 뭐가
    있는지 미리 알려주는 앱이라, "22분 더 걸린다"를 보여주고 고르는 건 사용자 몫으로 둔다.

    safe 자리에 늘 부담 최저가 온다고 전제하지 않는다 — 실측에서 부담 36점 경로가 35.9점을
    제치고 추천으로 뜬 적이 있다. 여기서 받은 두 개를 직접 견준다 (부르는 쪽을 믿지 않는다).
  */
  const recommendedRoute = 무의미한차이 ? "single" : safe.total < fast.total ? "safe" : "fast";
  ...
  const lead = 홀로
    ...
    : !fastIsQuicker
      ...
      : recommendedRoute === "fast"
        ? `빠른 경로의 추천점수가 더 높음 (${fastScore} / ${safeScore})`
        : `안심 경로의 추천점수가 더 높음 (${safeScore} / ${fastScore})`;
