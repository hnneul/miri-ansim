...
// AI는 그 결과를 읽어 한국어로 옮긴다. 그래서 추천점수도 근거 카드의 수치도 AI 때문에
...
//   radio    경로별 **출발 전 음성 안내 대본** 2~6칸 — 눈이 아니라 귀로 듣는 자리다
//
// radio 가 나머지 셋과 다른 점은 매체다. 화면은 숫자를 보여주고(추천점수 58, 좌회전 12번 → 3번)
 ...
 *
 * 6000 이었는데 그 값으로는 15번 중 1번만 들어왔다 — 나머지는 전부 여기서 잘렸다.
 * 실측(2026-08-17, ai.smoke.ts 5회 × 3호출): 5.2~9.9초, 중앙 8.6초. 응답이 느린 게 아니라
 * 출력이 크다 — 요약·브리핑 3줄·판정 2개·대본 두 벌을 한 번에 받는다.
 *
 * 화면은 이 시간을 안 기다린다. 규칙 대본이 이미 재생 가능한 상태로 앉아 있고
 * (app/route/page.tsx aiScript), 재생 중에 도착해도 읽던 대본을 끝까지 읽는다
 * (RouteRadio 재생()). 그래서 늦게 오는 값보다 **못 오는 값**이 비싸다.
 */
 ...
 * ("B 경로는 추천점수가 70.6으로…"). 화면에 나갈 이름을 그대로 키로 쓰면 그 문제가 없다.
 */
export type AiSentences = {
  summary: string;
  briefing: string[];
  verdicts: string[];
  /** 경로별 대본. 바깥이 경로 순서(verdicts 와 같다), 안이 칸(①오프닝 ②추천이유 ③각오 ④안내 ⑤도착 ⑥맺음말) */
  radio: string[][];
};
...
type RouteFacts = {
  ...
  소요시간분: number | null;
  거리km: number | null;
  /** 높을수록 권할 만한 길. 아래 요인 목록과 방향이 반대다 (lib/score.ts 추천점수 주석) */
  추천점수: number;
  /**
   * 부담이 큰 순서. 요인별 점수는 일부러 넣지 않는다 — 모델이 한쪽 경로의 요인 점수를
   * 다른 경로 요인에 붙이는 걸 봤다("28.2점인 좁은 교행 구간(평화로 1.1km)",
...
export type Facts = {
   ...
   * 실제 경로 데이터와 매핑되는 값은 점수 가중치에도 들어가고, 이 원래 라벨은 AI가 무엇을
   * 먼저 설명할지 정하는 데 쓴다. 주차처럼 경로 점수에 없는 선택도 도착 안내에는 필요하다.
...
- 각 경로의 "요인"은 부담이 큰 순서로 나열돼 있다. 요인별 점수는 주지 않았으니 말하지 않는다.
  점수는 경로 단위 값(추천점수 필드)만 쓴다. **추천점수는 높을수록 권할 만한 길이다** —
  낮은 쪽을 두고 "점수가 낮아 안전하다"처럼 거꾸로 말하지 않는다.
- 사실에 없는 위험요인(사고 이력, 경사, 날씨, 단속 등)은 언급하지 않는다.
- 운전 조언은 각 요인의 "행동수칙" 문장을 근거로만 말한다. 직접 만들지 않는다.
- 추천경로가 왜 추천인지는 summary 에서 추천점수와 소요시간으로 설명한다.
...
- "부담유형"은 이 사람이 **출발 전에 직접 고른** 부담이다. 실제 데이터가 있는 유형은 점수에도
  반영되지만, 길 설명에서도 우선순위를 맞춘다. 요인 하나를 골라 말하는 자리에서 경로에
  짝이 맞는 요인이 **있으면 그것을 먼저 고른다**. 없으면 평소대로 부담이 가장 큰 요인을 고른다.
  짝: "좁은 골목길"→좁은 교행 구간 · "급경사·굽은 길"→연속 급커브 ·
  "어두운 길"→연속 급커브 ·
  "주차 어려운 곳"→도착해서 차를 댈 곳(대본 ⑤칸)을 생략하지 않는다.
  "복잡한 교차로"는 현재 경로 점수 데이터에 짝이 없다 — 아무것도 바꾸지 않는다.
...
- radio: [{"경로": "경로 이름", "대본": ["①", "②", …]}, …]. 경로마다 하나씩.
  ...
  **번호나 기호를 문장에 쓰지 않는다.** 아래 ①②③④ 는 칸의 역할을 설명하려고 붙인 것이지
  ...
  ① **누구에게 하는 말인지 짚고, 오늘 어디를 어느 길로 가는지.**
     ...
     **이름 속 숫자가 수가 아닌 도로는 소리 나는 대로 적는다** — 귀로 듣는 글이라
     "516로"라고 쓰면 "오백십육로"로 읽힌다. 516로 → 오일육로, 1100로 → 천백로.
     ("서성일로10번길" 처럼 번지에서 온 이름은 그대로 둔다 — 그건 수로 읽는 게 맞다.)
  ② **왜 이 길인지.** 다른 경로와 비교해서 말한다 — 여기서만 다른 길을 언급해도 된다.
     **다른 경로를 도로 이름으로 부르지 않는다.** 이름을 모르는 사람에게는 뜻이 없다.
     **화면 카드가 그 길에 붙인 이름 그대로 "일반 길"이라고 부르고** 그 길의 성격으로 설명한다
     ("일반 길은 굽이가 계속 이어지는 산길인데, 이 길은 그게 없어요"). 화면에는 추천된 길이
     "맞춤 안심 길", 나머지가 "일반 길"로 떠 있다 — 듣는 말과 보는 말이 같아야 한 길 얘기로 읽힌다.
     "저쪽"처럼 가리키는 말도 쓰지 않는다 — 소리만으로는 무엇을 가리키는지 모른다.

     **"비교"의 좌회전·회전교차로를 추천 이유로 쓰지 않는다.** 그 값들은 추천점수에 들어가지
     않아서, 화면이 큰 글씨로 띄운 점수와 음성이 서로 다른 축으로 말하게 된다. 이유는 각
     경로의 "요인"(급커브·좁은 교행·고속주행)에서 고른다 — 일반 길에는 있고 이 길에는 없거나
     훨씬 덜한 요인 하나다.
  ...
  ④ **알아두면 좋은 것.** "비교"에 회전교차로가 있으면 이 칸을 쓴다. 없으면 이 칸이 없다.
     **부담이라고 말하지 않는다** — 회전교차로는 신호 교차로보다 안전한 시설이고 추천점수에도
     안 들어간다. 피하라는 말이 아니라 **들어가는 법**을 알려주는 자리다.
     예: "회전교차로가 있어요. 안에서 돌고 있는 차가 먼저니까, 비었을 때 들어가시면 돼요."
     나쁜 예(부담으로 말했다): "회전교차로가 두 곳이나 있어서 좀 까다로운 길이에요."
  ⑤ **도착해서 차를 댈 곳.** 사실에 "도착"이 있으면 이 칸을 쓴다 — 빼먹지 않는다.
     다만 **할 말이 이름뿐이면 이 칸을 쓰지 않는다.** 주차형태가 null 이고 요금이 무료가 아니고
     도보 시간도 없으면, 아래 규칙을 다 지키고 나서 남는 건 주차장 이름 하나뿐이다. 그러면
     "도착하면 ○○이에요." 한 줄이 되는데 그건 목적지를 다시 말하는 것일 뿐 듣는 사람이
     이미 아는 말이다. 이 칸의 값어치는 **대는 법을 미리 아는 것**이지 이름이 아니다.
     (나쁜 예: "도착하면 오설록 티뮤지엄이에요." — 이럴 바엔 ⑤칸이 없는 게 낫다.)
...
const SCHEMA = {
  ...
  properties: {
    ...
    // 칸 수를 2~6으로 열어둔 건 ②(추천 접음)·③(요인 없음)·④(안내 없음)·⑤(주차장 없음)가
    // 빠질 수 있어서다.
    radio: {
      type: "array",
      items: {
        ...
        properties: {
          경로: { type: "string" },
          대본: { type: "array", items: { type: "string" }, minItems: 2, maxItems: 6 },
        },
        required: ["경로", "대본"],
        additionalProperties: false,
      },
...
function 다른경로만의요인(facts: Facts, 이름: string): string[] {
  ...
  if (!이경로) return []; // 추천이 "없음"(점수 차이 작음)이면 두 경로를 다 말해도 된다
...
async function openai(prompt: string): Promise<unknown | null> {
  ...
  try {
    const res = await fetch(OPENAI_ENDPOINT, {
      ...
      body: JSON.stringify({
        model: OPENAI_MODEL,
        // temperature 를 못 준다 — 이 모델은 0 을 400 으로 거절한다
        // ("does not support 0 with this model. Only the default (1) value is supported").
        // 그래서 같은 입력에 같은 문장이라는 보장은 모델이 아니라 **캐시**가 진다 (아래 캐시 주석).
        // 서버가 다시 뜨면 문구가 달라질 수 있고, 숫자와 요인은 verify() 가 그대로 잡는다.
...
/**
 * 프롬프트 → 결과 캐시. 같은 화면을 두 번 그리는 데 두 번 부를 이유가 없고,
 * **같은 입력에 같은 문장이라는 보장을 지는 것도 여기다** (모델은 temperature 를 못 받는다 —
 * 위 openai 주석).
 *
 * 프롬프트에 실시간 소요시간이 들어가므로 교통이 바뀌면 자연히 다시 부른다 —
