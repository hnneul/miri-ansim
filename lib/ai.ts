...
- radio: [{"경로": "경로 이름", "대본": ["①", "②", …]}, …]. 경로마다 하나씩.
  ...
  **번호나 기호를 문장에 쓰지 않는다.** 아래 ①②③④ 는 칸의 역할을 설명하려고 붙인 것이지
  ...
  ① **누구에게 하는 말인지 짚고, 이 길이 어떤 길인지** 한 마디.
     "운전자조건"을 가정형이 아니라 **단정형으로 호명**한다. 이미 받아둔 정보라 "~라면"으로
     말하면 남 얘기처럼 들린다. 조건이 여럿이면 무거운 것 **둘까지**만 쉼표로 잇는다.
     조건이 하나도 없으면 호명을 통째로 생략하고 길 얘기부터 한다.
     예: "운전 경력 1년 미만, 제주는 처음이시죠. 평화로 타고 갈게요. 큰길 위주라 주변 차가 빨라요."
  ② **왜 이 길인지.** 다른 경로와 비교해서 말한다 — 여기서만 다른 길을 언급해도 된다.
     ...
     숫자를 옮겨 적는 게 아니라 그게 운전자에게 무엇인지를 말한다.
       ...
       좋은 예: "좌회전으로 맞은편 흐름을 끊고 들어가는 순간이 그만큼 줄어요."
     ...
     - 추천경로가 **아닐 때: 설득하지 않는다.** 이미 이 길을 고른 사람에게 하는 말이다.
       ...
       "익숙한 운전자에게 알맞은 선택이에요"처럼, 같은 사실을 탓하지 않는 말로 쓴다.
       예: "시간은 짧지만 긴장 구간이 더 많아요." / "익숙한 운전자에게 알맞은 선택이에요."
     - 추천경로가 "없음"일 때: 두 길이 비슷하다고 말하고 무엇을 맞바꾸는지만 알려준다.
  ③ 그래도 **각오할 것 하나**. 그 경로에서 부담이 가장 큰 요인 하나만 골라 그 "행동수칙"을
     짧게 쓴다. "부담설명"은 ①이 이미 한 마디로 했으니 되풀이하지 않는다.
     그 경로에 요인이 하나도 없으면 **이 칸을 쓰지 않는다**.
     예: "급커브와 좁은 교행 구간에서 속도를 줄이세요."
  ④ **도착해서 차를 댈 곳.** 사실에 "도착"이 있으면 **반드시 이 칸을 쓴다** — 빼먹지 않는다.
     ...
     예(주차형태 평행·확인됨 false·무료·도보 4분): "매일올레시장 공영주차장은 무료예요.
     평행주차일 가능성이 높으니 대는 법 미리 보고 가세요. 목적지까진 걸어서 4분이에요."
     나쁜 예(주차형태를 빼먹었다): "매일올레시장 공영주차장에 도착하면 도보 4분 정도 필요합니다."

  말투: 조수석에 앉은 아는 사람이 알려주듯. **"~해요" 체로 통일한다** —
