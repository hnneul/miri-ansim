 ...
 *
 * 테두리만 원본 2px 이 아니라 1.5px 이다. 그만큼 r 을 8 → 8.25 로 키워 바깥 지름 18 을 지켰다
 * (테두리는 선 한가운데를 기준으로 그려져서, 굵기를 줄이면 원이 같이 작아진다).
 *
 * **색만 회색으로 바꿨다.** 원본은 #FFC5AB / #FF773E 인데, 카카오 지도가 가게 이름을 주황
 * 글씨로 써서 우리 핀 40개가 지도 원래 글자와 안 갈렸다. 회색은 지도가 안 쓰는 색이라
 * 우리 것만 떨어져 나오고, 고른 하나만 주황이라 색으로도 유일해진다.
 *
 * 고른 것만 50x70 물방울에 로고를 넣는다. 40개가 다 같은 캐시백 가맹점이라 하나하나가 무엇인지
 * 말할 게 없고, 지도가 할 일은 "이 동네에 이만큼 깔려 있다"다. 로고는 고른 하나에만 붙어
 * 그게 어디인지 말한다 — 얼굴은 눈이 자동으로 쫓는 형태라 40개가 깔리면 기호보다 시끄럽다.
 *
...
const PIN = pin(
  `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 18 18">
     <circle cx="9" cy="9" r="8.25" fill="#c4c4c4" stroke="#8a8a8a" stroke-width="1.5"/>
   </svg>`,
);
