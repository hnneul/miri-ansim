 ...
 * 우리 것만 떨어져 나오고, 고른 하나만 주황이라 색으로도 유일해진다.
 *
 * 고른 것만 50x70 물방울에 로고를 넣는다. 20개가 다 같은 캐시백 가맹점이라 하나하나가 무엇인지
 * 말할 게 없고, 지도가 할 일은 "이 동네에 이만큼 깔려 있다"다. 로고는 고른 하나에만 붙어
 * 그게 어디인지 말한다 — 얼굴은 눈이 자동으로 쫓는 형태라 20개가 깔리면 기호보다 시끄럽다.
 *
 * 동그라미는 지도 위 점이니 **한가운데**가 그 가게 자리고, 물방울은 뾰족한 끝이 자리다.
 * 그래서 아래 기준점이 둘이 다르다.
 *
 * off 를 파일이 아니라 인라인 data: URI 로 두는 이유 — 원 하나짜리라 파일로 둘 이유가 없고,
 * **파일로 두면 고칠 때마다 브라우저 이미지 캐시에 걸린다** (모양을 바꿔도 옛 핀이 그대로 뜬다).
 * 인라인이면 소스가 바뀔 때 URI 자체가 바뀌어 그 일이 없다.
 *
 ...
 * 그려진 자리를 직접 넘겨 막는다 — 원은 한가운데 (9,9), 물방울은 뾰족한 끝 (28,69) 이고,
 * 물방울 쪽은 그림자가 번질 자리 때문에 캔버스가 그림보다 커서 환산한 값이다.
 */
const PIN_SIZE = [18, 18, 9, 9] as const;
