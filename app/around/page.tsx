...
// import 하면 그게 통째로 폰으로 내려간다 — 반경 안 20곳만 actions.ts 에서 받아 온다.
 ...
 * 20곳만 찍으니 가까운 20곳이 100m 안에 다 들어와, 지도는 1.5km를 보여주는데 핀은 한 덩어리로
 * 뭉쳐 서로를 가린다. 그래서 반경이 아니라 **실제로 찍는 20곳이 퍼진 범위**에 축척을 맞춘다.
 *
 * 그 20번째 거리는 데이터가 있어야 나오므로 서버가 실어 보낸다 (actions.ts fitM).
 ...
 * 20곳이 30m 안에 있는 골목이 있는데, 거기 맞춰 당기면 건물 몇 채만 남고 어디인지 알 수 없다.
 */
const MIN_FIT_M = 150;
 ...
 * 화면에 담을 폭 = 반경 × 이 값.
 *
 * 2.0(=지름)이면 20곳이 전부 화면에 들어오지만 시내에서는 핀이 서로를 덮는다.
 * 1.4 면 가장자리 몇 곳이 화면 밖으로 나가는 대신 나머지가 읽을 만해진다 —
 * 정확한 답은 어차피 목록이고(20곳 전부), 지도가 할 일은 "이 동네에 이만큼 깔려 있다"다.
 */
const FIT_SPAN = 1.4;
...
function Around() {
   ...
   * 세로 187px 안에 반경이 들어가도록만 당겨져서, 가로로는 1km가 펼쳐지고 핀 20개가 화면
   * 한구석에 뭉친다. 접고 시작하면 띠가 3배로 넓어져 그만큼 더 당겨지고 핀이 퍼진다.
   *
   * 카드에 이름·업종·도보 시간뿐이라(원본에 사진도 메뉴도 없다) 목록을 앞세울 이유도 약하다.
   * "내 주변에 캐시백 되는 데가 있나"에 답하는 건 이름 20개가 아니라 핀이 깔린 지도다.
   */
   ...
   * 돌아오는 건 많아야 20곳이라 2KB 쯤이다 — 11,912곳 838KB 를 폰이 들고 있던 것과 바꾼 값이다.
   *
  ...
  //     START_LEVEL 그대로였다 — 핀 20개가 한 덩어리로 뭉친 채로.
  ...
  return (
    <aside
      /*
        ...
        풀 때 목록 20개가 잠깐 떴다가 쓸려 내려간다 — 실제로 그렇게 보였다. 둘을 맞추려면
 ...
 *                글자 대신 **탐나는전 로고(모자 쓴 얼굴)** 가 들어간다 — 하나만 뜨는 자리라
 *                얼굴을 써도 안 시끄럽고, 무슨 앱의 가맹점인지까지 말한다.
 *
 * 20개가 다 같은 캐시백 가맹점이라 하나하나가 무엇인지 말할 게 없다. 지도가 할 일은
 ...
 * **파일로 두면 고칠 때마다 브라우저 이미지 캐시에 걸린다** (모양을 바꿔도 옛 핀이 그대로 뜬다).
 */
...
/**
 * 고른 핀만 **파일**이다 (public/tamna-pin-selected.svg).
 *
 * 그림은 /parking 의 PIN_ON 과 같은 점토 물방울인데, 글자 자리에 탐나는전 로고가 들어간다.
 * 그 로고가 자동 벡터화로 딴 것이라 좌표만 6KB다 — 소스에 인라인으로 박으면 이 파일을
 * 사람이 읽을 수 없게 된다. 한 번에 하나만 뜨는 그림이라 요청 한 번은 그 값을 한다.
 *
 * 파일명이 tamna-pin-on 이 아니라 -selected 인 이유 — 같은 이름으로 두면 전에 그 경로로
 * 받아간 옛 핀이 브라우저 이미지 캐시에 남아 그대로 뜬다. 이름을 바꾸는 게 캐시를 비우는
 * 가장 싼 방법이다 (이 화면에서 실제로 겪었다).
 */
const PIN_ON = "/tamna-pin-selected.svg";
