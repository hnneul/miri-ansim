...
/**
 * Edge 로 떨어졌을 때의 목소리 (1순위는 아래 gemini 의 Kore 다).
 *
 * 후보 스물넷을 같은 대본으로 뽑아 듣고 고른 값이다
 ...
 *
 * 대신 이 갈래에서는 문장 **안**에서 억양이 오르내리지 않는다. 그건 LLM 기반 음성 모델만
 * 하는 일이고(1순위가 그쪽이다), 재래식 뉴럴 음성에는 그 손잡이가 없다.
 ...
 * 대본 → 음성 캐시.
 *
 * 1순위 음성이 소리 길이로 값을 매기므로(아래 GEMINI_MODEL) 캐시가 곧 예산이다.
 * 같은 칸을 두 번 만들 이유도 없고 재생이 즉시 시작되는 값어치도 크다.
 * 브라우저 캐시(아래 Cache-Control)가 한 겹 더 받는다.
 */
 ...
 *
 * **2.5 로 내려봤다가 되돌렸다 (2026-08-20).** 출력 단가가 정확히 절반($10/1M)이라 공짜
 * 이득처럼 보였는데, **짧은 칸을 두 번 읽는다.** 실측(브라우저 resource 항목의 바이트/글자):
 *   보통 칸(25~57자)              630~726 B/자
 *   "도착하면 엉또폭포예요."(12자)      2,080 B/자
 *   "도착하면 오설록 티뮤지엄이에요."(17자) 2,033 B/자
 * 같은 칸을 3.1 로 만들면 815 B/자다. 세 배로 부는 건 2.5 쪽에서만 난다.
 *
 * 짧은 칸은 없앨 수 없다 — ⑥칸(맺음말)은 고정 한 마디라 늘 13자 안팎이다 (lib/ai.ts).
 * ⑤칸은 재료가 이름뿐일 때 빼도록 규칙을 조였지만 ⑥칸은 남는다. 그래서 값을 포기했다.
 * 반값이라도 맺음말이 두 번 읽히면 안내가 끝난 줄을 모른다.
 *
 * 다시 재볼 거면 **캐시 두 겹을 반드시 피해라.** 캐시 키가 대본 텍스트뿐이라(아래 `캐시`)
 * 모델을 바꿔도 같은 대본이면 옛 오디오가 그대로 나온다 — 브라우저는 `max-age=86400` 이라
 * 더 질기다. 실제로 이걸 못 피해서 "3.1 인데도 두 번 읽는다"고 한참 헤맸다.
 * **처음 보는 경로 + 시크릿 창**이 제일 싸게 피하는 법이다.
 *
 * "2.5-pro 로 내리면 된다(값이 같다)"고 적어뒀던 옛 주석도 틀렸다 — Pro 는 Flash 보다 비싸다.
 * 즉 이 모델에서 **값으로 내려갈 곳이 없다.**
 */
 ...
 * 떨어지고, 사용자는 목소리가 바뀐 것만 느낀다. 대신 **한도도 값도 없다** — 1순위(gemini)가
 * 키·권한·타임아웃 어디서 넘어져도 여기가 받는다.
 */
