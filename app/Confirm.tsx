 ...
 *
 * **onOk 를 안 주면 버튼 하나짜리 알림 창이 된다.** 브라우저 alert() 를 여기로 데려오려고 뒀다 —
 * confirm() 을 피한 이유가 그대로 alert() 에도 해당한다(폰 밖에서 뜨고 도메인 줄이 붙는다).
 * 물을 게 없으니 취소가 없고, 어둠막을 눌러도 닫히는 건 같다.
 *
...
export default function Confirm({
  ...
  title: string;
  body: string;
  /** 닫기. 알림 창(onOk 없음)에서는 확인 버튼도 이걸 부른다 */
  onCancel: () => void;
  /** 없으면 알림 창이 된다 */
  onOk?: () => void;
  okLabel?: string;
