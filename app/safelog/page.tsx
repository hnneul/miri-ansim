 ...
 *
 * 거리·시간·경로·추천점수·"왜 안심 길이었나요?" 세 줄은 전부 길 비교 화면이 매긴 값 그대로다
 * (app/route/page.tsx 담기 → lib/safelog.ts SafeDrive). 여기서 다시 계산하지 않는다.
 */
 ...
 * 와이어프레임 목록에 그려진 네 건.
 *
 * **깔아두지 않는다.** 아직 아무것도 안 담은 사람에게 네 건을 채워두면 화면이 거짓말을 한다
 * (메인화면 "여행 기록"과 같은 규칙). 그래서 처음 들어오면 빈 화면(2770:2050)이 뜬다 —
 * 지금 상태를 그대로 말하는 화면이다.
 *
 * 시연에서 목록·탭·카드·상세를 한 번에 보여주려고 `?demo=1` 일 때만 이 둘이 담긴다.
 *
...
function Safelog() {
   ...
   * 실제로 담으려면 길 비교에서 내비로 한 번 넘어가야 하는데(app/route/page.tsx 담기),
   * 시연에서 목록·카드·상세를 바로 보여주려면 채워둔 판이 하나 필요하다.
   */
