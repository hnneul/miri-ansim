...
/**
 * 이만큼은 나가 있어야 "내비를 열어봤다"고 친다. 누르자마자 마음이 바뀌어 돌아온 사람에게
 * 말을 걸면 성가시다. visibilitychange 는 알림을 확인하러 잠깐 나갔다 와도 오므로,
 * 넘겼는지(떠남키)만으로는 못 거른다.
 *
 * **5분이었다가 5초로 내렸다.** 5분은 "실제로 주행하고 돌아왔나"를 재는 값이었다. 그러려면
 * 아래 문구가 "운전은 괜찮으셨나요?"처럼 다녀온 걸 단정해야 하는데, 그 단정이 안 맞는 자리가
 * 너무 많았다 — 지도만 확인하고 오는 사람, 시연으로 눌러보는 사람.
 *
 * 문구를 **사실형**으로 바꾸면서(담아뒀다는 안내) 이 값이 재는 것도 달라졌다. 이제는
 * "정말 운전했나"가 아니라 **"누르자마자 되돌아온 오조작인가"** 하나다. 담기는 버튼을 누르는
 * 순간 이미 끝나므로(담기), 언제 떠도 문구는 참이다 — 남은 건 성가심뿐이고 5초면 충분하다.
 * 잘못 눌러 1~2초 만에 되돌아온 사람에게는 안 뜨고, 카카오맵을 한 번 보고 온 사람에게는 뜬다.
 */
const 다녀온최소 = 5 * 1000;
...
function Route() {
   ...
   *
   * 시간을 안 채웠으면 안 띄우고 **적어둔 시각도 안 지운다** — 나중에 다시 오면 그때 뜬다.
   * 지워 버리면 잠깐 돌아온 한 번에 기회가 통째로 날아간다.
   */
