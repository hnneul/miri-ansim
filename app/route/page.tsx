...
function Route() {
   ...
   * 여기서 await 하면 10초 안팎으로 시트가 "길을 살펴보는 중"에 머문다.
   *
   ...
   *
   * **다녀왔다고 단정하지 않는다.** 우리가 아는 건 "내비를 열었고 돌아왔다"까지고, 그 사이에
   * 진짜 운전을 했는지는 알 길이 없다 — 지도만 보고 온 사람에게 "운전은 괜찮으셨나요?"는
   * 앱이 없던 일을 지어낸 말이 된다. 그래서 일어난 일만 말한다: 담아뒀다는 것.
   * 이 앱의 "모르면 침묵" 규칙이 문구에도 그대로 걸리는 자리다.
   *
   * **얼마나 나가 있었는지는 안 잰다.** 예전에는 5분(뒤엔 5초)을 채워야 띄웠는데, 그 문턱은
   * 문구가 다녀온 걸 단정하던 시절의 방어였다. 사실만 말하는 지금은 0.5초 만에 돌아와도 참이고,
   * 잘못 눌러 되돌아온 사람이야말로 "담겼다"를 알아야 지우러 갈 수 있다 ("보러가기"가 그 문이다).
   */
  ...
  useEffect(() => {
    const 돌아옴 = () => {
      ...
      if (!sessionStorage.getItem(떠남키)) return;
  ...
  return (
    <div className="relative flex flex-1 flex-col overflow-hidden bg-white">
      ...
      {/* 비교와 근거가 같은 라우트라 이름도 상태를 따라간다 — 지도가 머리인 화면이라 글자는 안 그린다 */}
      <h1 className="sr-only">{view === "why" ? "안심 길 근거" : "길 비교"}</h1>
        ...
        */}
        ...
        {!dest ? (
          ...
          <>
              ...
              {collapsed && chosen ? (
                ...
                */
                ...
                <>
                  ...
                  길이 한 장인 경우(alone)는 lib/briefing.ts 가 빈 문자열을 주고, 그러면 이
            ...
            >
              {/*
                **앱에서 검정 버튼은 흐름에 들어오기 전 화면**(스플래시 "프로필 만들기" ·
                온보딩 "다음")에만 쓰고, 흐름 안에서 하려던 일은 전부 주황이다
                (출발·근처 주차장 보기, 자세히·여기로 갈게요). 와이어프레임은 여기를 검정으로
                뒀지만(2153:1730) 그러면 결이 어긋난다.
